...
// Top-level function
export async function getIdenticalAndSimilar(
	components: SignComponents
): Promise<IdenticalAndSimilar> {
...
export async function getSimilarVariants(
	...
	async function findCharacter(characterInput: SignComponents): Promise<ComparableVariant[]> {
		...
		return foundVariants.map((foundVar) => ({
			category: foundVar.category,
			character: foundVar.character
		}));
	}

	const keyFound = await Promise.all(
		getTypedKeys(hypotheticalVariants).map(async (key) => ({
