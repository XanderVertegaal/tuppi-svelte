<script lang="ts">
  import { browser } from "$app/environment";
	import type { characters_char, characters_char$result, CompVariants$result, FontSet$options } from "$houdini";
  ...
  function getBaseChar(compVariant: CompVariants$result['compVariants']['identicalVariants'][0]): BaseCharacter | null {
    ...
    const categoryVariant = compVariant.character.variants?.find(variant => variant.category === category);
    if (categoryVariant) {
      return {
  ...
  function getBaseCharacters(identicalVariants: CompVariants$result['compVariants']['identicalVariants'] | null): BaseCharacter[] {
  ...
  function getDisplaySimilarCharacters(similarCharacters: SimilarCharacters | null): DisplaySimilarVariants {
    ...
    <section class="gallery">
      <ul>
        {#each baseCharacters as baseChar}
          <li>
            <SignCard fontSet={baseChar.fontSet} character={baseChar.character} enableSelect={false}/>
          </li>
        {/each}
      </ul>
    </section>
    ...
    <tr>
      <td>
        {#each similarCharacters.verticalMinus as variant}
          <span class="{fontsetMapping[variant.fontSet]}">{renderUnicode(variant.character.unicode)}</span>
        {/each}
      </td>
